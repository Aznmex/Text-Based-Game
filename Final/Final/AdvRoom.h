...
public:
	...
	string getDescription();	   // Return the room description.
	string getName();					   // Returns the room name.

	void addObject(AdvObject obj);		   // Adds an object to the room.
	void removeObject(string objName);// Removes an object with name objName and returns the object.
	...
	vector<AdvObject>getObjects();		// vector for getting objects
	void dropObject(AdvObject obj);		// drops the object 
